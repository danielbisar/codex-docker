...
- `auth.md` documents the manual device-auth flow and how to mount saved Codex credentials.
...
For authentication docs, follow `auth.md` in a fresh container and confirm that mounting `home_codex/` avoids repeated device authentication.
...
Treat `home_codex/` as sensitive local state because it can contain authentication data and logs. Do not commit credentials, copied Codex state, or generated SQLite/log files. If changing user IDs, mounted paths, or sandbox flags, update `auth.md`.
